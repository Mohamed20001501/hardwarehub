  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="pb-4 border-b border-slate-100 space-y-0.5">
        <h2 className="text-xl font-extrabold text-slate-800">Orders Received</h2>
        <p className="text-slate-500 text-xs">Manage customer purchases, update delivery status, and coordinate fulfillment details.</p>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-slate-50 border border-slate-150 rounded-2xl p-10 text-center space-y-3 shadow-inner">
          ...
          <p className="text-slate-500 text-xs max-w-xs mx-auto">Once customers add your hardware products to their cart and checkout, their orders will appear here.</p>
        </div>
      ) : (
            ...
            return (
              <div key={order._id} className="bg-white border border-slate-150 rounded-2xl p-5 shadow-sm space-y-4 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2">
                  <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest">Items Purchased</p>
                  <ul className="text-sm text-slate-700 space-y-2 divide-y divide-slate-100/50">
